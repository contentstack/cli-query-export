...
import { QueryResolver } from '../utils/query-resolver';
import { DependencyResolver, DependencyResolverConfig } from './dependency-resolver';
...
const setupConfig = async (exportCmdFlags: any): Promise<ExportConfig> => {
  ...
  if (Array.isArray(config.filteredModules) && config.filteredModules.length > 0) {
    config.modules.types = filter(defaultConfig.modules.types, (module: any) =>
      includes(config.filteredModules, module),
    );
  }

  // Setup query configuration
  if (exportCmdFlags.query) {
    const queryResolver = new QueryResolver(config);
    const resolvedQuery = await queryResolver.resolveQuery(exportCmdFlags.query);

    // Handle reference flags
    let includeReference = false;
    let skipReference = false;

    if (exportCmdFlags['include-reference']) {
      includeReference = true;
    } else if (exportCmdFlags['skip-reference']) {
      skipReference = true;
    } else {
      // Interactive prompt if neither flag is provided
      const response = (await cliux.inquire({
        type: 'confirm',
        name: 'includeReference',
        message: 'Do you want to include referenced content (entries, assets, global fields, etc.)?',
        default: false,
      })) as { includeReference: boolean };
      includeReference = response.includeReference;
      skipReference = !includeReference;
    }

    config.queryResolver = queryResolver;
    config.isQueryBasedExport = true;
    config.includeReference = includeReference;
    config.skipReference = skipReference;

    // Setup dependency resolver configuration
    const dependencyResolverConfig: DependencyResolverConfig = {
      originalModules: config.modules.types,
      queryResolver: queryResolver,
      includeReference: includeReference,
      skipReference: skipReference,
    };

    config.dependencyResolverConfig = dependencyResolverConfig;
  }
